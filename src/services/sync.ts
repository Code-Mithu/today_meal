...
type PushResult = { operationId: string; accepted: boolean; error?: string };
...
async function meta(key: string) { return (await queryFirst<{ value: string }>('SELECT value FROM sync_meta WHERE key = ?', [key]))?.value ?? null; }
async function setMeta(key: string, value: string) { await execute('INSERT OR REPLACE INTO sync_meta (key, value) VALUES (?, ?)', [key, value]); }
...
async function stageLocalChanges(householdId: string, localGroupId: string) {
  for (const table of TABLES) {
    const rows = table === 'groups' ? await queryAll<Record<string, unknown>>('SELECT * FROM groups WHERE id = ?', [localGroupId]) : await queryAll<Record<string, unknown>>(`SELECT * FROM ${table} WHERE group_id = ?`, [localGroupId]);
...
async function applyChange(change: Change) {
  if (change.entity_type === 'audit_logs') {
    const entries = Object.entries(change.payload).filter(([key]) => /^[a-z_]+$/.test(key));
    await execute(`INSERT OR REPLACE INTO audit_logs (${entries.map(([key]) => key).join(', ')}) VALUES (${entries.map(() => '?').join(', ')})`, entries.map(([, value]) => typeof value === 'object' && value !== null ? JSON.stringify(value) : value as never));
    return;
  }
  ...
  if (change.deleted_at) {
    ...
    await execute(`INSERT OR REPLACE INTO ${table} (${entries.map(([key]) => key).join(', ')}) VALUES (${entries.map(() => '?').join(', ')})`, entries.map(([, value]) => typeof value === 'object' && value !== null ? JSON.stringify(value) : value as never));
  }
...
export async function synchronize(localGroupId: string, householdName: string) {
  ...
  const pending = await queryAll<OutboxRow>('SELECT * FROM sync_outbox WHERE attempts < 8 ORDER BY created_at LIMIT 250');
  if (pending.length) {
    try {
      const result = await apiFetch<{ results: PushResult[] }>('/api/sync/push', { method: 'POST', body: JSON.stringify({ householdId, operations: pending.map((row) => ({ operationId: row.operation_id, entityType: row.entity_type, entityId: row.entity_id, baseVersion: row.base_version, deleted: Boolean(row.deleted), payload: JSON.parse(row.payload) })) }) });
      for (const item of result.results) {
        if (item.accepted || !item.error) await execute('DELETE FROM sync_outbox WHERE operation_id = ?', [item.operationId]);
        else await execute('UPDATE sync_outbox SET attempts = attempts + 1, last_error = ? WHERE operation_id = ?', [item.error, item.operationId]);
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Sync request failed';
      for (const row of pending) await execute('UPDATE sync_outbox SET attempts = attempts + 1, last_error = ? WHERE operation_id = ?', [message, row.operation_id]);
      throw error;
    }
  }
  let cursor = Number(await meta('sync_cursor') ?? 0); let hasMore = true;
  while (hasMore) {
    ...
    await runInTransaction(async () => { for (const change of result.changes) await applyChange(change); await setMeta('sync_cursor', String(result.cursor)); });
    cursor = result.cursor; hasMore = result.hasMore;
  }
  await setMeta('last_sync_at', new Date().toISOString());
  return { pending: (await queryFirst<{ count: number }>('SELECT COUNT(*) as count FROM sync_outbox'))?.count ?? 0 };
}
